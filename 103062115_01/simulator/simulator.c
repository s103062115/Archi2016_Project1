...
void Sra(int rt,int rd,int C){
    unsigned int a = 0;
    int i;
    if(sign(registers[rt]) == 1){
        for(i = 0;i < C;i++){
            a = a+Pow(2,31-i);
        }
    }
    registers[rd] = registers[rt] >> C;
    registers[rd] = registers[rd]+a;
}
...
void Lw(int rs,int rt,int C){
    ...
    if((int)registers[rs] + A1 > 1020 || (int)registers[rs] + A1 < 0){
        error_ao = 1;
    }
...
void Lhu(int rs,int rt,int C){
    ...
    if((int)registers[rs] + C > 1022 || (int)registers[rs] + C < 0){
        error_ao = 1;
    }
    if(error() == 0){
...
void Lh(int rs,int rt,int C){
    ...
    if(((int)registers[rs] + A1)%2!=0){
        error_dm = 1;
    }else if((int)registers[rs] + A1 > 1022 || (int)registers[rs] + A1 < 0){
        error_ao = 1;
    }
    if(error() == 0){
